...
export default {
  ...
  watch: {
    dialogVisible(newVal) {
      if (newVal) {
        ...
        if (JSON.stringify(this.ZzdbgData) != "{}") {
          console.log("this.yglbData", this.ZzdbgData);
          //   this.formData = JSON.parse(JSON.stringify(this.yglbData)); //深拷贝，避免影响表格
          this.formData = {
            dictvalue: this.ZzdbgData.dictvalue,
            parentvalue: this.ZzdbgData.parentvalue,
            dictid: this.ZzdbgData.dictid,
            descriptions: this.ZzdbgData.description,
            shortdesc: this.ZzdbgData.shortdesc,
            id: this.ZzdbgData.id
          };
        }
      }
    }
  ...
  methods: {
    submitForm(formData) {
      ...
      if (this.editTypeZ == "add") {
        let params = {
          description: this.formData.descriptions,
          dictid: this.dictid,
          dictvalue: this.descriptionName,
        ...
        ZAddDictitemDict(params).then(res => {
          if (res && res.code) {
            ...
            // 关闭弹窗
            this.dialogClose();
            this.$emit("refresh", this.formData.dictid);
          } else {
            this.$message.error("保存失败");
          }
        });
      }
      if (this.editTypeZ == "edit") {
        let params = {
          description: this.formData.descriptions,
